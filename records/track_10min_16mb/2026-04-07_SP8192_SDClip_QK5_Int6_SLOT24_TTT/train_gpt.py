...
class Hyperparameters:
    ...
    tied_embed_lr = float(os.environ.get("TIED_EMBED_LR", 0.02))  # Lower LR for 8× larger embedding table
    tied_embed_init_std = float(os.environ.get("TIED_EMBED_INIT_STD", 0.003))  # Tighter init for 8192 vocab
    matrix_lr = float(os.environ.get("MATRIX_LR", 0.022))  # From PR #1331 frontier
    ...
    muon_wd = float(os.environ.get("MUON_WD", 0.085))  # From PR #1331/#1394 frontier
    adam_wd = float(os.environ.get("ADAM_WD", 0.04))
    qat_enabled = bool(int(os.environ.get("QAT_ENABLED", "0")))
    bigram_vocab_size = int(os.environ.get("BIGRAM_VOCAB_SIZE", 0))  # Disabled: SP8192 vocab absorbs n-gram info
    bigram_dim = int(os.environ.get("BIGRAM_DIM", 0))
    trigram_enabled = bool(int(os.environ.get("TRIGRAM", "0")))  # Disabled with SP8192
    ...
    ve_enabled = bool(int(os.environ.get("VE_ENABLED", "0")))  # Disabled: SP8192 tok_emb already captures value identity
    ...
    # Depth Recurrence: re-run specified layers after full forward pass
    recur_layers = os.environ.get("RECUR_LAYERS", "4,5")  # PR #1394/#1420: layers 4-5 proven optimal
    enable_looping_at = float(os.environ.get("ENABLE_LOOPING_AT", 0.35))  # PR #1420: activate at 35% wallclock (adapts to actual throughput)
    recur_warmup_steps = int(os.environ.get("RECUR_WARMUP_STEPS", 20))
    num_loops = int(os.environ.get("NUM_LOOPS", 3))  # PR #1420: triple loop = layers 4-5 run 3 extra times
    # Context-Only SLOT (Sample-specific LM Optimization at Test-time)
    ...
    parallel_residual_n = int(os.environ.get("PARALLEL_RESIDUAL_N", 4))  # GPT-J style parallel attn+MLP for last N layers
    # Score-first TTT (test-time training)
    ttt_enabled = bool(int(os.environ.get("TTT_ENABLED", "1")))
    ttt_lr = float(os.environ.get("TTT_LR", 0.005))
    ttt_epochs = int(os.environ.get("TTT_EPOCHS", 3))  # PR #1413 uses 3 epochs

# --- Batched Newton-Schulz orthogonalization ---

def zeropower_via_newtonschulz5(G: Tensor, steps: int = 5, eps: float = 1e-7) -> Tensor:
    """Batched Newton-Schulz orthogonalization with MuonEq-R+C normalization. G: (B,M,N) or (M,N)."""
    ...
    if transposed:
        X = X.mT
    # MuonEq-R+C: row-normalize then column-normalize before NS5
...
class CastedLinear(nn.Linear):
    ...
    _soft_round_alpha: float = 1.0  # Soft-Round QAT alpha (ramps 1→16)
    def forward(self, x: Tensor) -> Tensor:
        ...
        if CastedLinear._qat_enabled and self.training and w.ndim == 2:
            ...
            # soft_round(x) = floor(x) + sigmoid(alpha * (x - floor(x) - 0.5))
            # As alpha → ∞, this converges to round(x)
...
class Block(nn.Module):
    def __init__(
        ...
        self.parallel_residual = False  # set True for GPT-J style parallel attn+MLP
        if dtg:
    ...
    def forward(self, x: Tensor, x0: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, up_w: Tensor, down_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        if self.parallel_residual:
            # GPT-J style: attn and MLP both see pre-attention input, errors don't compound through GPTQ
...
class GPT(nn.Module):
    def __init__(
        ...
        self.register_buffer('_recur_scale_t', torch.zeros(1))  # tensor buffer to avoid torch.compile recompilation
        ...
        # Parallel residuals (GPT-J style) for last N layers: reduces GPTQ error compounding
        pr_n = int(os.environ.get("PARALLEL_RESIDUAL_N", "4"))
        if pr_n > 0:
            for i in range(max(0, num_layers - pr_n), num_layers):
                self.blocks[i].parallel_residual = True
        self._init_weights()
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_decoder_layers):
            ...
            x, _ = self.blocks[bi](x, x0,
                ...
                v_embed=ve, v0=v0)
        # Depth recurrence: re-run specified layers (weight-shared virtual layers)
        if self.recur_layer_indices:
            x_pre = x
            for _loop in range(self.num_loops):
                for li in self.recur_layer_indices:
                    ve = self._get_ve(li, input_ids, ve_cache)
                    x, _ = self.blocks[li](x, x0,
                        self.qo_bank[li], self.kv_bank[li], self.kv_bank[n + li],
                        self.qo_bank[n + li], self.mlp_up_bank[li], self.mlp_down_bank[li],
                        v_embed=ve, v0=v0)
            x = torch.lerp(x_pre, x, self._recur_scale_t)
        x = self.final_norm(x)
    ...
    def forward_hidden(self, input_ids: Tensor) -> Tensor:
        """Return final hidden states (bsz, seq_len, dim) before logit projection."""
        n = self.num_layers
        x = self.tok_emb(input_ids)
        if self.bigram is not None:
            x = x + self.bigram(input_ids)
...
def eval_val_sliding_slot(
    ...
    slot_steps: int = 16,
    slot_lr: float = 0.008,
    slot_lr_min: float = 0.0008,
    batch_seqs: int = 8,
    eval_seq_len: int | None = None,
) -> tuple[float, float]:
    """Context-Only SLOT sliding window evaluation.

    For each window, optimizes per-sample additive delta (on hidden states)
    and logit bias using ONLY the context tokens (positions 0..seq_len-stride-1).
    Then scores the stride tokens (positions seq_len-stride..seq_len-1)
    with the adapted delta + bias. This is strictly causal: the optimization
    never sees the tokens being scored.
    """
    seq_len = eval_seq_len or args.train_seq_len
    total_tokens = val_tokens.numel() - 1
    window_starts = [ws for ws in range(0, total_tokens, stride)
    ...
    # Warm-start: running average of optimized logit_bias across batches.
    # Consecutive windows overlap by (seq_len - stride) tokens, so biases transfer well.
    warm_bias = torch.zeros(1, 1, vocab_size, device=device, dtype=torch.float32)
    for bi in range(0, len(my_windows), batch_seqs):
        batch_ws = my_windows[bi:bi + batch_seqs]
        bsz = len(batch_ws)
        x_batch = torch.zeros(bsz, seq_len, dtype=torch.int64, device=device)
        ...
        for i, ws in enumerate(batch_ws):
            ...
            x_batch[i, :wlen] = chunk[:-1]
            y_batch[i, :wlen] = chunk[1:]
        # Step 1: Get frozen hidden states (no grad for model weights)
        with torch.no_grad(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            hidden = base_model.forward_hidden(x_batch)  # [bsz, seq_len, dim]
        hidden = hidden.detach().float()
        # Get the projection weight once (detached from model)
        if base_model.tie_embeddings:
            w = base_model.tok_emb.weight.detach().float()
        else:
            w = base_model.lm_head.weight.detach().float()
        # Pre-compute context end positions for this batch
        ctx_ends: list[int] = []
        for i, ws in enumerate(batch_ws):
            wlen = wlens[i]
            ctx_ends.append(0 if ws == 0 else max(wlen - stride, 0))
        # Step 2: Optimize per-sample delta and logit bias on context tokens only
        delta = torch.zeros(bsz, 1, model_dim, device=device, dtype=torch.float32, requires_grad=True)
        logit_bias = warm_bias.expand(bsz, -1, -1).clone().detach().requires_grad_(True)
        opt = torch.optim.AdamW([delta, logit_bias], lr=slot_lr, weight_decay=0.0, betas=(0.9, 0.999))
        # Check if all context lengths are identical (common case: skip per-sample loop)
        uniform_ctx = len(set(ctx_ends)) == 1 and ctx_ends[0] > 0
        for slot_step in range(slot_steps):
            # Cosine LR schedule
            cos_decay = 0.5 * (1 + math.cos(math.pi * slot_step / slot_steps))
            current_lr = slot_lr_min + (slot_lr - slot_lr_min) * cos_decay
            for pg in opt.param_groups:
                pg["lr"] = current_lr
            opt.zero_grad(set_to_none=True)
            adapted = hidden + delta  # [bsz, seq_len, dim] broadcast
            logits_proj = adapted @ w.T + logit_bias  # [bsz, seq_len, vocab]
            logits = softcap * torch.tanh(logits_proj / softcap)
            # Loss ONLY on context tokens (not scored tokens)
            if uniform_ctx:
                # Batched: all samples have identical context length — single kernel
                ce = ctx_ends[0]
                loss = F.cross_entropy(logits[:, :ce, :].reshape(-1, vocab_size),
                                       y_batch[:, :ce].reshape(-1), reduction="mean")
                loss.backward()
                opt.step()
            else:
                context_losses = torch.zeros(bsz, device=device, dtype=torch.float32)
                valid_count = 0
                for i in range(bsz):
                    if ctx_ends[i] > 0:
                        context_losses[i] = F.cross_entropy(
                            logits[i, :ctx_ends[i], :], y_batch[i, :ctx_ends[i]], reduction="mean")
                        valid_count += 1
                if valid_count > 0:
                    loss = context_losses.sum() / valid_count
                    loss.backward()
                    opt.step()
        # Update warm bias from this batch's optimized logit_bias
        with torch.no_grad():
            warm_bias = logit_bias.detach().mean(dim=0, keepdim=True)
        # Step 3: Score stride tokens with adapted hidden + bias (no grad)
        with torch.no_grad():
            adapted = hidden + delta.detach()
            logits_proj = adapted @ w.T + logit_bias.detach()
            logits = softcap * torch.tanh(logits_proj / softcap)
            nll = F.cross_entropy(
            ...
            for i in range(bsz):
                ...
                tb += (has_leading_space_lut[tgt] & ~is_boundary_token_lut[prev]).to(torch.float64)
                byte_count += tb.sum()
        del delta, logit_bias, opt, hidden
    if dist.is_available() and dist.is_initialized():
        dist.all_reduce(loss_sum, op=dist.ReduceOp.SUM)
        dist.all_reduce(token_count, op=dist.ReduceOp.SUM)
        dist.all_reduce(byte_count, op=dist.ReduceOp.SUM)
...
def eval_val_ttt(
    ...
    """Score-first test-time training evaluation.

    For each batch of non-overlapping windows:
      1. SCORE stride tokens with frozen model (inference mode)
      2. TRAIN model on the full window (gradient update)
    This is legal: each token is scored BEFORE any training on its window.
    The model progressively adapts to the val distribution.
    """
    seq_len = eval_seq_len or args.train_seq_len
    total_tokens = val_tokens.numel() - 1
    # Non-overlapping windows for TTT
    ...
    # Lightweight optimizer: only update linear weights, not embeddings or norms
    ttt_params = []
    for name, p in base_model.named_parameters():
        if p.ndim >= 2 and "tok_emb" not in name and "bigram" not in name and "ve_" not in name:
            p.requires_grad_(True)
            ttt_params.append(p)
        else:
            p.requires_grad_(False)
    opt = torch.optim.SGD(ttt_params, lr=ttt_lr, momentum=0.9, weight_decay=0.01)
    base_model.eval()
    ...
    for wi, ws in enumerate(my_windows):
        ...
        # STEP 1: Score stride tokens FIRST (frozen)
        score_start = max(wlen - stride, 0)
        with torch.no_grad(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            logits = base_model.forward_logits(x)  # [1, wlen, vocab]
        logits_f = logits.float().squeeze(0)
        if softcap > 0:
            pass  # softcap already applied in forward_logits
        nll = F.cross_entropy(logits_f[score_start:wlen], y.squeeze(0)[score_start:wlen],
                              reduction="none").to(torch.float64)
        loss_sum += nll.sum()
        scored_count = wlen - score_start
        token_count += float(scored_count)
        tgt = y.squeeze(0)[score_start:wlen]
        prev = x.squeeze(0)[score_start:wlen]
        tb = base_bytes_lut[tgt].to(torch.float64)
        tb += (has_leading_space_lut[tgt] & ~is_boundary_token_lut[prev]).to(torch.float64)
        byte_count += tb.sum()
        del logits, logits_f, nll

        # STEP 2: Train on full window AFTER scoring
...
def quantize_int6_per_row(t: Tensor, clip_range: int = 31, sdclip_k: float = 12.85) -> tuple[Tensor, Tensor]:
    """SDClip: clip = k * std(row) replaces slow quantile search."""
    t32 = t.float()
    if t32.ndim == 2:
        row_clip = (sdclip_k * t32.std(dim=1)).clamp_min(1e-8)
...
def quantize_int6_gptq(weight, hessian=None, clip_range=31, block_size=128, sdclip_k=12.85):
    """Full GPTQ: Hessian-aware int6 quantization with Cholesky error compensation.
    Uses SDClip (k * std) for scale computation instead of percentile search.
    If hessian is None, falls back to SDClip per-row."""
    t32 = weight.float()
    ...
    # SDClip: single scale computation (replaces percentile search loop)
    row_clip = (sdclip_k * t32.std(dim=1)).clamp_min(1e-8)
    s = (row_clip / clip_range).clamp_min(1.0 / clip_range).to(torch.float16)
    sf = s.float()
...
def _quantize_int6_percentile(t32, clip_range=31, sdclip_k=12.85):
    """SDClip fallback for 1D or no-Hessian cases."""
    if t32.ndim == 2:
        row_clip = (sdclip_k * t32.std(dim=1)).clamp_min(1e-8)
        s = (row_clip / clip_range).clamp_min(1.0 / clip_range).to(torch.float16)
        q = torch.clamp(torch.round(t32 / s.float()[:, None]), -clip_range, clip_range).to(torch.int8)
...
class _HessianGPT(nn.Module):
    ...
    def __init__(self, vocab_size, num_layers, model_dim, num_heads, num_kv_heads,
        ...
        self.recur_layer_indices = [int(x) for x in recur_layers.split(",") if x.strip()] if recur_layers else []
        self.num_loops = num_loops
        self.register_buffer('_recur_scale_t', torch.ones(1))  # always fully active for calibration
        self.tok_emb = nn.Embedding(vocab_size, model_dim)
        self.bigram = BigramHashEmbedding(bigram_vocab_size, bigram_dim, model_dim, trigram=bool(int(os.environ.get("TRIGRAM", "1")))) if bigram_vocab_size > 0 else None
        self.smear = SmearGate(model_dim)
        self.num_encoder_layers = num_layers // 2
    ...
    def forward(self, input_ids, target_ids):
        ...
        # Depth recurrence: mirror main GPT forward for accurate Hessian calibration
        if self.recur_layer_indices:
            x_pre = x
            for _loop in range(self.num_loops):
                for li in self.recur_layer_indices:
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str], hessians: dict[str, Tensor] | None = None,
                        weight_sdclip_k: float = 12.85, embed_sdclip_k: float = 20.0):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            ...
            k = embed_sdclip_k if cat == "embed" else weight_sdclip_k
            H = hessians.get(name) if hessians else None
            if H is not None:
                q, s = quantize_int6_gptq(t, hessian=H, clip_range=cr, sdclip_k=k)
            else:
                q, s = quantize_int6_per_row(t, clip_range=cr, sdclip_k=k)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6"}
        else:
...
def main() -> None:
    ...
    recur_activated_step = 0  # will be set on first step where wallclock >= enable_looping_at
    ...
    while True:
        ...
        # Soft-Round alpha ramp: 1 → 16 over 500 steps after QAT starts
        if CastedLinear._qat_enabled:
            qat_steps_elapsed = step - qat_start_step
            alpha_ramp = min(qat_steps_elapsed / 500.0, 1.0)
            CastedLinear._soft_round_alpha = 1.0 + 15.0 * alpha_ramp
        # Depth recurrence warmup: off until enable_looping_at wallclock fraction, then ramp over recur_warmup_steps
        if base_model.recur_layer_indices:
            looping_threshold_ms = args.enable_looping_at * max_wallclock_ms if max_wallclock_ms is not None else 0.0
            if elapsed_ms < looping_threshold_ms:
                base_model._recur_scale_t.fill_(0.0)
                recur_activated_step = step + 1  # reset: not yet activated
            else:
                steps_since = step - recur_activated_step
                if steps_since < args.recur_warmup_steps:
                    base_model._recur_scale_t.fill_(steps_since / args.recur_warmup_steps)
                else:
                    base_model._recur_scale_t.fill_(1.0)
        zero_grad_all()
    ...
    if args.lawa_enabled and len(lawa_queue) > 1:
        ...
        base_model.load_state_dict(avg_state, strict=True)
    base_model._recur_scale_t.fill_(1.0)  # full recurrence for eval/export
    torch.cuda.synchronize()
    ...
    hessian_model = _HessianGPT(
        ...
        recur_layers=args.recur_layers, num_loops=args.num_loops,
    ).to(device).bfloat16()
    ...
    quant_result, quant_meta = mixed_quantize_int6(unbanked_sd, {"mlp", "attn"}, hessians=hessians)  # Embeddings use int8 (better precision for 8192 vocab)
    ...
    eval_model._recur_scale_t.fill_(1.0)  # full recurrence during eval
    ...
    if args.slot_enabled:
        ...
        torch.cuda.synchronize()
        log0(
            f"final_int6_slot val_loss:{slot_val_loss:.4f} val_bpb:{slot_val_bpb:.4f} "
            f"stride:{slot_stride} steps:{args.slot_steps} lr:{args.slot_lr} "
            f"eval_time:{1000.0 * (time.perf_counter() - t_slot):.0f}ms"
        )
        log0(f"final_int6_slot_exact val_loss:{slot_val_loss:.8f} val_bpb:{slot_val_bpb:.8f}")
    # Score-first TTT: legal test-time training (each window scored BEFORE training)
    if args.ttt_enabled:
        ttt_model = copy.deepcopy(eval_model)
        ttt_model._recur_scale_t.fill_(1.0)
        torch.cuda.synchronize()
        t_ttt = time.perf_counter()
        ttt_val_loss, ttt_val_bpb = eval_val_ttt(
            args, ttt_model, rank, world_size, device,
            val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
            stride=args.train_seq_len,  # non-overlapping windows
            ttt_lr=args.ttt_lr,
